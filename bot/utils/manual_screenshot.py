...
class ManualScreenshot:
    async def manual_screenshot_fn(self, c, m):
        ...
        try:
            async with timeout(Config.TIMEOUT), ProcessCounter(
                ...
                if isinstance(duration, str):
                    ...
                    log_msg = await media_msg.forward(Config.LOG_CHANNEL)
                ...
                if len(valid_positions) > 10:
                    await snt.edit_text(
                        f"😟 Sorry! Only 10 screenshots can be generated. Found {len(valid_positions)} "
                ...
                if invalid_positions:
                    ...
                    txt = (
                        f"Found {len(invalid_positions)} invalid positions ({invalid_position_str}).\n\n"
                ...
                await snt.edit_text(
                    text=f"🤓 You requested {len(valid_positions)} screenshots and "
                    f"{len(screenshots)} screenshots generated, "
            ...
            await snt.edit_text(
                "😟 Sorry! Video trimming failed due to timeout. Your process was taking "
                "too long to complete, hence cancelled"
            )
            ...
            log_msg = await media_msg.forward(Config.LOG_CHANNEL)
            await log_msg.reply_text(
                f"manual screenshots ({raw_user_input}) where requested and some "
