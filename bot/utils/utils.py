...
class CommonUtils:
    ...
    async def generate_thumbnail_file(file_path, uid):
        ...
        ffmpeg_cmd = ["ffmpeg", '-headers', f'IAM:{Config.IAM_HEADER}', "-ss", "0", "-i", f"'{file_link}'", "-vframes", "1", "-vf", '"scale=320:-1"',  '-y', thumb_file]
    ...
    async def get_media_info(file_link):
        ffprobe_cmd = ['ffprobe', '-headers', f'IAM:{Config.IAM_HEADER}', '-v', 'quiet', '-of', 'json', '-show_streams', '-show_format', '-show_chapters', '-show_programs', f"'{file_link}'"]
        data, err = await CommonUtils.run_subprocess(ffprobe_cmd)
        return data

    @staticmethod
    async def get_dimentions(file_link):
        ffprobe_cmd = ['ffprobe', '-headers', f'IAM:{Config.IAM_HEADER}', '-v', 'error', '-show_entries', 'stream=width,height', '-of', 'csv=p=0:s=x', '-select_streams', 'v:0', f"'{file_link}'"]
    ...
    async def get_duration(file_link):
        ffmpeg_dur_cmd = ['ffprobe', '-headers', f'IAM:{Config.IAM_HEADER}', '-v', 'error', '-show_entries', 'format=duration', '-of', 'csv=p=0:s=x', '-select_streams', 'v:0', f"'{file_link}'"]
    ...
    async def fix_subtitle_codec(file_link):
        ...
        ffmpeg_dur_cmd = ["ffprobe", '-headers', f'IAM:{Config.IAM_HEADER}', '-v', 'error', '-select_streams', 's', '-show_entries', 'stream=codec_name', '-of', 'default=noprint_wrappers=1:nokey=1', f"'{file_link}'"]
