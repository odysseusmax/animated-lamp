    ...
    async def screenshot_fn(self, c, m):
        ...
        try:
            async with timeout(Config.TIMEOUT), ProcessCounter(
                ...
                if not screenshots:
                    await m.edit_message_text(
                        text="😟 Sorry! Screenshot generation failed possibly due to some infrastructure failure 😥."
                    )
                    log_msg = await media_msg.forward(Config.LOG_CHANNEL)
                    if ffmpeg_errors:
                        ...
                        await log_msg.reply_text(
                            ...
                            " where requested and Screen shots where not generated.",
                            True,
                        )
                    return
            ...
            await m.edit_message_text(
                text="😟 Sorry! Video trimming failed due to timeout. Your process was "
