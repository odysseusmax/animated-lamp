        ...
        try:
            async with timeout(Config.TIMEOUT) as cm:
                ...
                for i, sec in enumerate(screenshot_secs):
                    ...
                    ffmpeg_cmd = ['ffmpeg', '-hide_banner', '-ss', str(sec), '-i', shlex.quote(file_link), '-vf', f'"drawtext=fontcolor={watermark_color}:fontsize={fontsize}:x={x_pos}:y={y_pos}:text=\'{shlex.quote(watermark)}\', scale=1280:-1"', '-y', '-vframes', '1', str(thumbnail_template)]
