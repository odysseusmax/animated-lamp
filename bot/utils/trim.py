        ...
        try:
            async with timeout(Config.TIMEOUT), ProcessCounter(
                ...
                if request_duration > Config.MAX_TRIM_DURATION:
                    await m.reply_text(
                        f"Please provide any range that's upto {Config.MAX_TRIM_DURATION}s."
                        f" Your requested range **{start}:{end}** is `{request_duration}s` long!",
                        True,
                    )
                    return
            ...
            await snt.edit_text(
                "😟 Sorry! Video trimming failed due to timeout. Your process was taking too "
                "long to complete, hence cancelled"
            )
            ...
            log_msg = await media_msg.forward(Config.LOG_CHANNEL)
            await log_msg.reply_text(
                f"trim video requested and some error occoured\n\n{traceback.format_exc()}",
                True,
            )
        finally:
