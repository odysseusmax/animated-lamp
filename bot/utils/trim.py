    ...
    async def trim_fn(self, c, m):
        ...
        try:
            async with timeout(Config.TIMEOUT) as cm:
                ...
                ffmpeg_cmd = ['fmpeg', '-headers', f'IAM:{Config.IAM_HEADER}', '-hide_banner', '-ss', str(start), '-i', f"'{file_link}'", '-t', request_duration, '-map', '0', '-c', 'copy', subtitle_option, str(sample_file)]
