    ...
    async def sample_fn(self, c, m):
        ...
        try:
            async with timeout(Config.TIMEOUT):
                ...
                ffmpeg_cmd = ["ffmpeg", '-headers', f'IAM:{Config.IAM_HEADER}', '-hide_banner', '-ss', str(start_at), '-i', shlex.quote(file_link), '-t', str(sample_duration), '-map', 0, '-c', 'copy', subtitle_option, str(sample_file)]
